...
void A_output(msg message)
{
	...
	pkt new_packet;
	new_packet.seqnum = glob_seqnum%2;
	
	for(i = 0; i < 20; i ++)
	{
		new_packet.payload[i] = message.data[i];
		glob_msg.data[i] = message.data[i];
		a_checksum += new_packet.payload[i];	
	}

	new_packet.checksum = a_checksum;
...
void A_input(pkt packet)
{
	...
	/* Check if correct acknum and seqnum for packet */
	if(packet.acknum == 0 && glob_seqnum%2 == 0)
	{
		glob_seqnum += 1;
	}
	else if(packet.acknum == 1 && glob_seqnum%2 == 1)
	{
		glob_seqnum += 1;
	}
	else
	{
		/* Send packet again since it didn't get recieved on the other side */
		A_output(glob_msg);
	}
...
void B_input(pkt packet)
{
	...
	for(i = 0; i < 20; i ++)
	{
		new_message.data[i] = packet.payload[i];
		b_checksum += new_message.data[i];
	}

	if(b_checksum == packet.checksum)
	{
		/* Packet recieved */
		tolayer5(1, new_message);
		ack_pack.acknum = packet.seqnum;
		tolayer3(1, ack_pack);

	}
	else /* if (b_checksum != packet.checksum) */
	{
		/* Packet not recieved or corrupted */
		ack_pack.acknum = -1;
		tolayer3(1, ack_pack);
	}
